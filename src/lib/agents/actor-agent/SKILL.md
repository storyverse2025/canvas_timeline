...
**单一句**：`Rendering style: {{visualStyle}}` —— 把全局美术风格这段
文字原样写进 prompt 即可。

**禁止**叠加额外的相机 / 镜头 / 引擎 / 分辨率术语（Sony Venice /
Panavision / Final Fantasy CG / Unreal Engine 5 / 8K / Kodak Vision3
等）。这些会把每张图都拽向「写实摄影」基线，覆盖用户在导演助手里
选定的风格（例如「都市灵异动漫」、「水墨」、「赛博朋克」）。如果
项目就是写实摄影风，那它已经写进全局 visualStyle 里了，不用再叠。

### Pillar 7 · 防 AI 提示词 (Anti-AI tropes — negative directives)

**根据角色具体处境写 2-4 条针对性负面提示**，而不是整段复制下面这
张参考清单。例如：
- 反派 → 选 1-2 条："不做漫画式邪笑"、"不做美型反派模板（银发苍白
  瞳孔小恶魔）"
- 童星 → 选 1-2 条："不做夸张大眼无神"、"不做欧美高挺鼻型"
- 写实老者 → 选 1-2 条："不做过度磨皮"、"不做油亮水光肌"

参考清单（**不要整段贴**，只挑与本角色相关的）：
