...
- **每行 scene + character 人数上限**：一行只能有 1 个 scene；角色放进 `characters` 数组，至多 6 个（群戏/女团同框就把所有出镜成员都列进同一行的 `characters`，不要拆）。只有当某行出镜角色 >6 或跨 2 个场景时才拆成多行，重新分配 duration 使总和仍等于 {{totalDurationSeconds}}；scene 字段填实际所在场景。
- 确保修复后的景别分布合理
- 保持角色和场景的连续性
