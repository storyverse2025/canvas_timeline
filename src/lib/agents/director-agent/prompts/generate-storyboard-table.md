...
- "RAPID CUTS" / "FLASH IMAGES" 通常列出 3-5 个子镜头（典型每个 1-2 秒）。把每个子镜头展开成独立的 row（最低 2s，相邻的 RAPID CUTS 几行 row 可以共享 scene 上下文）。**不要**把它们合并成"群像快剪段"一个 row。
- 用户在剧本里对镜头数量已经给出明确暗示时（"系列镜头 ABC = 3 个"、"凯/陆/空 三人各一段战斗 = 3 个"），分配 shot count 优先匹配用户暗示，再让 totalDurationSeconds 约束最终时长。
- **反摘要硬约束**：如果"医生诊断后剧本"看起来已经是【0-20s】这种时间码段落格式（说明上游 script-agent 出了 bug 没保留分镜），你仍然要尝试从原剧本结构推断出合理的镜头切分，而不是直接拿时间段当 row。

【医生诊断后剧本（权威基准，所有镜头必须忠实于它，不要再次"扩写"或"总结"它，只是把它转成分镜；剧本里如果有显式分镜标记，必须 1 个标记 = 1 个 row）】
{{revisedScript}}
