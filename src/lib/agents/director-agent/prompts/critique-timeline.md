...
- **主动合并同场景过碎 row（重点）**：扫描相邻 row，凡是**同一场景（同一地点/空间）内连续、且都 < 10s** 的若干 row，默认判为"过碎"，必须开一条 issue 要求合并为单个 10-15s row（每个原 row 成为该 row 内的一格/cut，内容/运镜逐格保留，总时长重算后仍锁定）。fix 要直接点名要合并哪几行，例如 `fix: "S4/S5/S6 同在咖啡馆且各 3-4s，合并为单个 12s row，三个机位作为三格"`。例外：用户显式快剪 / RAPID CUTS / 明确强节奏点可保留 < 10s。
...
- **前后衔接（transition_note）校验**：
  - 相对上一行**换了场景**的行：transition_note 是否写了开头过渡手法（对白关键词呼应/匹配剪辑/场景穿越/动作衔接等）+ **上一行结尾是否保留了 ~1s 留白**；缺失或空泛 = 一条 issue。
  - 相对上一行**同场景连续**的行：transition_note 是否写了画面构图衔接（机位/景别/视线/角色位置如何承接上一行收尾画面）；缺失或空泛 = 一条 issue。
  - 第一行 transition_note 是否写了开场处理。
