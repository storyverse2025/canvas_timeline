...
|---|---|---|
| `shoot(req)` | Build a Seedance prompt from `row.motion_prompts` + `row.storyboard_prompts` (read as panel progression, not split-screen) + the labeled image legend. Calls `text-to-video` pinned to `doubao-seedance-2-0-260128 @ 480p`. Duration clamped to [5, 15]. Returns `{ url, prompt, durationSeconds, refs }`. | `useStoryboardGenerate.generateBeatVideo`'s hand-rolled prompt construction |
