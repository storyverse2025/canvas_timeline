...
import { routerPlugin } from './vite-router-plugin'
...
export default defineConfig({
  plugins: [react(), ...(useHttps ? [basicSsl()] : []), libtvPlugin(), providersPlugin(), capabilitiesPlugin(), sessionSnapshotPlugin(), routerPlugin()],
  server: {
    host: '0.0.0.0',
    port: 8080,
    proxy: {
